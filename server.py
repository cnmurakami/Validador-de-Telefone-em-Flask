...
def telefone():
    ...
    if len(request.args.keys())==2:
        ...
        lista_erros=[]
        if not len(ddd)==2 or ddd not in lista_ddd.keys() or len(telefone) < 8 or len(telefone) > 9 or (len(telefone) == 9 and not telefone[0] == '9') or (len(telefone) == 8 and telefone[0] == '9'):
            if not len(ddd)==2:
            ...
            if len(telefone) == 8 and telefone[0] == '9':
                lista_erros.append('Telefones celulares necessitam de 9 dígitos')
